...
const QUALITY_TIERS = [
  { name: '720p', width: 1280, height: 720, fps: 24, bitrate: 1_000_000 },
  { name: '540p', width: 960, height: 540, fps: 20, bitrate: 600_000 },
  { name: '360p', width: 640, height: 360, fps: 15, bitrate: 350_000 }
];
...
let currentQualityIndex = 1; // Start at 540p to avoid an initial latency spike; auto-quality can move to 720p.
...
// Optional call features. Screen sharing reuses the same WebCodecs/WebSocket
// video path, so the backend does not need any new packet types.
let displayStream = null;
let sharingScreen = false;
let screenShareBtn = null;
let screenShareLabel = null;
let pipBtn = null;
let pipLabel = null;
let pipVideo = null;
let pipCanvasStream = null;
let callHintTimer = null;
...
function activeVideoTrack() {
  if (sharingScreen) {
    ...
    if (displayTrack?.readyState === 'live') return displayTrack;
  }
  return stream?.getVideoTracks?.()[0] || null;
}

function canSendVideo() {
  return sharingScreen || cameraEnabled;
}

function showCallHint(text, duration = 3500) {
  if (!els.latencyText) return;
  clearTimeout(callHintTimer);
  els.latencyText.textContent = text;
  callHintTimer = setTimeout(() => {
    if (latestRtt > 0) els.latencyText.textContent = `Server round trip ~${latestRtt} ms`;
    else els.latencyText.textContent = 'Secure transport ready';
  }, duration);
}

function updateScreenShareUi() {
  if (!screenShareBtn || !screenShareLabel) return;
  screenShareBtn.classList.toggle('active', sharingScreen);
  screenShareBtn.setAttribute('aria-pressed', String(sharingScreen));
  screenShareBtn.setAttribute('aria-label', sharingScreen ? 'Stop sharing screen' : 'Share screen');
  screenShareLabel.textContent = sharingScreen ? 'Stop share' : 'Share';
  els.localVideo.classList.toggle('screen-sharing', sharingScreen);
  els.localVideo.classList.toggle('camera-disabled', !cameraEnabled && !sharingScreen);
  els.localCameraOff.classList.toggle('hidden', sharingScreen || cameraEnabled);
  const youLabel = document.querySelector('.you-label');
  if (youLabel) youLabel.textContent = sharingScreen ? 'Your screen' : 'You';
}

async function stopScreenShare(notifyPeer = true) {
  if (!sharingScreen && !displayStream) return;

  const oldDisplayStream = displayStream;
  displayStream = null;
  sharingScreen = false;

  // Clear state before stopping the track because "ended" can fire here too.
  oldDisplayStream?.getTracks?.().forEach(track => {
  ...
  if (notifyPeer && socket?.readyState === WebSocket.OPEN) {
...
async function toggleScreenShare() {
  if (sharingScreen) {
    ...
    showCallHint('Screen sharing stopped.');
    return;
  }

  if (!navigator.mediaDevices?.getDisplayMedia) {
    showCallHint('Screen sharing is not available in this browser.');
    return;
  }

  try {
    const nextDisplayStream = await navigator.mediaDevices.getDisplayMedia({
      video: {
        width: { ideal: 1280 },
        height: { ideal: 720 },
    ...
    const track = nextDisplayStream.getVideoTracks()[0];
    if (!track) throw new Error('No screen video track was returned.');

    try { track.contentHint = 'detail'; } catch {}

    displayStream = nextDisplayStream;
    sharingScreen = true;

    track.addEventListener('ended', () => {
      if (sharingScreen) stopScreenShare().catch(() => {});
    }, { once: true });

    els.localVideo.srcObject = new MediaStream([track]);
    await els.localVideo.play().catch(() => {});
    ...
    if (socket?.readyState === WebSocket.OPEN) {
      // The relay already understands camera-state; "enabled" simply tells the
      // other side that video frames are about to arrive.
      socket.send(JSON.stringify({ type: 'camera-state', enabled: true }));
    }

    showCallHint('Sharing your screen.');
  } catch (error) {
    console.warn('Screen share failed:', error);
    if (error?.name !== 'NotAllowedError') {
      showCallHint('Could not start screen sharing.');
    }
  }
}

function setPipUi(active) {
  if (!pipBtn || !pipLabel) return;
  pipBtn.classList.toggle('active', active);
  pipBtn.setAttribute('aria-pressed', String(active));
  pipLabel.textContent = active ? 'Exit PiP' : 'PiP';
}

async function ensurePipSource() {
  if (!pipVideo) return false;
  if (pipVideo.srcObject) return true;
  if (typeof els.remoteCanvas.captureStream !== 'function') return false;

  pipCanvasStream = els.remoteCanvas.captureStream(30);
...
async function togglePiP() {
  if (peerCount < 2 || els.remoteCanvas.classList.contains('hidden')) {
    showCallHint('PiP is available once your friend’s video is showing.');
    return;
  }

  const hasSource = await ensurePipSource();
  if (!hasSource) {
    showCallHint('Picture in Picture is not supported here.');
    return;
  }
  ...
  try {
    ...
    // Standards path (Chrome/Edge and newer Safari builds).
    ...
    console.debug('Standard PiP path unavailable:', error);
  }

  // Apple's older Safari presentation-mode API.
  try {
    if (
      ...
      const nextMode = pipVideo.webkitPresentationMode === 'picture-in-picture'
        ? 'inline'
        : 'picture-in-picture';
      pipVideo.webkitSetPresentationMode(nextMode);
      setPipUi(nextMode === 'picture-in-picture');
      return;
    }
  } catch (error) {
    console.debug('Safari PiP path unavailable:', error);
  }

  showCallHint('PiP is not available for this live video on this Apple browser.');
}

function installExtraCallControls() {
  const actions = document.querySelector('.call-actions');
  if (!actions || screenShareBtn) return;

  // Keep six controls usable on smaller phones without editing style.css.
  const style = document.createElement('style');
  style.id = 'linkline-extra-controls-style';
  style.textContent = `
    .call-actions { flex-wrap: wrap; max-width: 640px; }
    .local-video.screen-sharing { transform: none; object-fit: contain; background: #050609; }
    .pip-helper {
      position: fixed; left: -8px; bottom: -8px; width: 2px; height: 2px;
      opacity: .001; pointer-events: none;
    }
    @media (max-width: 600px) {
      .call-actions { max-width: 360px; gap: 7px; }
      .call-control { flex: 0 0 68px; }
    }
  `;
  document.head.appendChild(style);

  screenShareBtn = document.createElement('button');
  screenShareBtn.id = 'screenShareBtn';
  screenShareBtn.className = 'call-control';
  screenShareBtn.type = 'button';
  ...
  screenShareBtn.innerHTML = `
    <span class="control-icon">
      <svg viewBox="0 0 24 24" aria-hidden="true">
        ...
        <path d="M8 21h8M12 17v4M8 10l4-4 4 4M12 6v7"></path>
      </svg>
    </span>
    <span id="screenShareLabel">Share</span>
  `;
  screenShareLabel = screenShareBtn.querySelector('#screenShareLabel');
  screenShareBtn.addEventListener('click', toggleScreenShare);

  pipBtn = document.createElement('button');
  pipBtn.id = 'pipBtn';
  pipBtn.className = 'call-control';
  pipBtn.type = 'button';
  ...
  pipBtn.innerHTML = `
    <span class="control-icon">
      <svg viewBox="0 0 24 24" aria-hidden="true">
        ...
        <rect x="12" y="11" width="7" height="5" rx="1"></rect>
      </svg>
    </span>
    <span id="pipLabel">PiP</span>
  `;
  pipLabel = pipBtn.querySelector('#pipLabel');
  pipBtn.addEventListener('click', togglePiP);

  // Put Share and PiP before the existing Full screen button.
  actions.insertBefore(screenShareBtn, els.fullscreenBtn);
  actions.insertBefore(pipBtn, els.fullscreenBtn);

  pipVideo = document.createElement('video');
  pipVideo.className = 'pip-helper';
  pipVideo.autoplay = true;
  pipVideo.muted = true;
  pipVideo.playsInline = true;
  pipVideo.setAttribute('playsinline', '');
  pipVideo.addEventListener('enterpictureinpicture', () => setPipUi(true));
  pipVideo.addEventListener('leavepictureinpicture', () => setPipUi(false));
  pipVideo.addEventListener('webkitpresentationmodechanged', () => {
    setPipUi(pipVideo.webkitPresentationMode === 'picture-in-picture');
  });
  document.body.appendChild(pipVideo);

  if (!navigator.mediaDevices?.getDisplayMedia) {
    screenShareBtn.title = 'Screen sharing is not supported by this browser';
  }
}
...
function createEncoder() {
  ...
  videoEncoder = new VideoEncoder({
    output: (chunk) => {
      if (!socket || socket.readyState !== WebSocket.OPEN || peerCount < 2 || !canSendVideo()) return;
      const tier = currentTier();
      // Prevent latency from ballooning if the connection is congested.
      if (socket.bufferedAmount > 64_000) return; // Drop stale video instead of building seconds of delay.
...
function decodeVideoPacket(buffer) {
  ...
  if (videoDecoder.decodeQueueSize > 3) {
    try { videoDecoder.reset(); } catch {}
    waitingForKeyFrame = true;
    requestRemoteKeyFrame();
  }
...
function encodeCurrentFrame(nowMs) {
  if (!videoEncoder || videoEncoder.state !== 'configured' || !canSendVideo() || peerCount < 2) return;
...
function adaptQuality() {
  ...
  const overloaded = buffered > 160_000 || latestRtt > 300 || videoEncoder.encodeQueueSize > 2;
  const healthy = buffered < 32_000 && (latestRtt === 0 || latestRtt < 170) && videoEncoder.encodeQueueSize <= 1;
  ...
  // Require sustained healthy transport before increasing quality again.
  if (qualityStableTicks >= 4 && currentQualityIndex > 0) {
    currentQualityIndex -= 1;
    qualityStableTicks = 0;
    configureEncoder();
...
function startAdaptiveQuality() {
  stopAdaptiveQuality();
  adaptiveTimer = setInterval(adaptQuality, 4000);
}
...
async function initMedia() {
  ...
  captureNode.port.onmessage = (event) => {
    ...
    // Audio must stay current. If the transport is badly backed up, drop stale
    // audio rather than letting speech arrive seconds late.
    if (event.data instanceof ArrayBuffer && socket.bufferedAmount < 192_000) {
...
function handlePeerCount(peers) {
  ...
  if (!live) {
    ...
    forceKeyFrame = true;
    socket?.send(JSON.stringify({ type: 'camera-state', enabled: cameraEnabled || sharingScreen }));
    requestRemoteKeyFrame();
...
function cleanupMedia() {
  ...
  pipCanvasStream?.getTracks?.().forEach(track => {
    try { track.stop(); } catch {}
  });
...
function leaveCall() {
  ...
  els.localVideo.classList.remove('camera-disabled', 'screen-sharing');
...
function toggleCamera() {
  ...
  els.localVideo.classList.toggle('camera-disabled', !cameraEnabled && !sharingScreen);
  els.localCameraOff.classList.toggle('hidden', cameraEnabled || sharingScreen);
  if (cameraEnabled || sharingScreen) forceKeyFrame = true;
  if (socket?.readyState === WebSocket.OPEN) {
    socket.send(JSON.stringify({ type: 'camera-state', enabled: cameraEnabled || sharingScreen }));
  }
}

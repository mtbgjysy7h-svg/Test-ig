...
const QUALITY_TIERS = [
  { name: '720p', width: 1280, height: 720, fps: 30, bitrate: 1_800_000 },
  { name: '540p', width: 960, height: 540, fps: 24, bitrate: 1_050_000 },
  { name: '360p', width: 640, height: 360, fps: 20, bitrate: 550_000 }
];
...
function encodeCurrentFrame(nowMs) {
  ...
  if (!stream || els.localVideo.readyState < 2 || videoEncoder.encodeQueueSize > 2) return;
  const tier = currentTier();
...
function adaptQuality() {
  ...
  const overloaded = buffered > 700_000 || latestRtt > 450 || videoEncoder.encodeQueueSize > 4;
  const healthy = buffered < 120_000 && (latestRtt === 0 || latestRtt < 220) && videoEncoder.encodeQueueSize <= 1;
  ...
  // Require ~20 seconds of healthy transport before moving back up a tier.
  if (qualityStableTicks >= 5 && currentQualityIndex > 0) {
    currentQualityIndex -= 1;
    qualityStableTicks = 0;
    configureEncoder();
...
async function initMedia() {
  ...
  captureNode.port.onmessage = (event) => {
    ...
    if (event.data instanceof ArrayBuffer && socket.bufferedAmount < 2_000_000) {

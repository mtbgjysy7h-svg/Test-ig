...
const wss = new WebSocketServer({ server, maxPayload: 300_000 });
...
function broadcastCount(roomId) {
  ...
  for (const peer of room.peers) {
    sendJson(peer, { type: 'peer-count', peers: room.peers.size });
  }
}
...
function cleanup(ws) {
  ...
  if (room.peers.size === 0) rooms.delete(info.roomId);
  else broadcastCount(info.roomId);
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (data, isBinary) => {
    ...
    if (info.frames > 180 || info.bytes > 3_200_000) return;
    ...
    if (message.type === 'ping') {
      sendJson(ws, { type: 'pong', at: message.at });
      return;
    }
    ...
    if (
      ...
      for (const peer of room.peers) {
        if (peer !== ws) sendJson(peer, relayed);
      }
      return;
    }

    if (message.type !== 'join' || info.joined) return;
    ...
    if (!/^[A-Z0-9-]{4,24}$/.test(roomId) || !/^[a-f0-9]{64}$/.test(proof)) {
      sendJson(ws, { type: 'error', message: 'Invalid room information.' });
    ...
    if (room.proof !== proof) {
      sendJson(ws, { type: 'error', message: 'Wrong passphrase for this room.' });
      ws.close(1008, 'wrong passphrase');
      return;
    }

    if (room.peers.size >= 2) {
      sendJson(ws, { type: 'error', message: 'This room already has two callers.' });
    ...
    sendJson(ws, { type: 'joined', peers: room.peers.size });
